...
void cmd_ping_on_ping_end(esp_ping_handle_t hdl, void *args)
{
	...
	// }
	if(received>0){
		ESP_LOGE(TAG," Is internet available=true");
		if(xSemaphoreTake(internetMutex, 1)==pdTRUE){
				is_internet_available=true;
				xSemaphoreGive(internetMutex);
				ESP_LOGI(TAG," Is internet available=true");
		}
	
		// ESP_LOGI(TAG," packets %d received,", received);
	}
	else{
		if(xSemaphoreTake(internetMutex, 1)==pdTRUE){
				is_internet_available=false;
				xSemaphoreGive(internetMutex);
				ESP_LOGI(TAG," Is internet available=false");
		}
		// ESP_LOGI(TAG, "NO internet %d packets transmitted, %d received,",transmitted, received);
	}
...
esp_err_t initialize_ping(uint32_t interval_ms, uint32_t task_prio)
{
	...
	// if (strlen(target_host) > 0) {
		...
		ping_config.tos=0;
		ping_config.task_stack_size=2500;
		ping_config.interface=0;
		target_addr.u_addr.ip4.addr=134744072;/*8.8.8.8 google ip address*/
		// ESP_LOGI("ping config", "pinging address:%s", ip4addr_ntoa(&(target_addr.u_addr.ip4)));
	...
	ping_config.count =/*ESP_PING_COUNT_INFINITE*/5;		// ping in infinite mode, esp_ping_stop can stop it
