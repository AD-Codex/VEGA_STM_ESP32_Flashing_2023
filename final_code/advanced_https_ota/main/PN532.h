...
/*!
 ...
 @modified by Vishva Oshada(T673)
           to read NFC cards of chargeNET
 @section  HISTORY
...
#define I2C_WRITE_TIMEOUT 100 // ms
#define I2C_READ_TIMEOUT 100  // ms
#define IRQ_WAIT_TIMEOUT 100  // ms

  // states and variables for PN532read()
  volatile uint8_t PN532ReadState;
  extern unsigned int NFCfaultCount;

#define CHECK_FIRMWARE 0
#define CONFIGURE_PN532 1
#define lIST_AVAILABLE_CARD 2
#define AUTHANTICATE 3
#define DATA_READ 4
#define END_READ 5
#define ONE_CYCLE_COMPLETE 6
#define FAULT 7
#define NFC_READY 8
#define PROCESS_OK 1
#define PROCESS_NOT_OK 0

  extern char userRefID[25];
  extern uint8_t data[16];
  // extern char userReferenceIDchr[25];
  // char userRefID[25];
  char userRefIDEmpty[25];
  uint8_t uid[] = {0, 0, 0, 0, 0, 0, 0}; // Buffer to store the returned UID
  uint8_t uidLength;
  uint8_t keya[6] = {0xFF, 0x33, 0x22, 0x44, 0xAA, 0xDD};

  uint8_t data[16] = {0};
  // char userReferenceIDchr[25];
  uint8_t nfc_err_cnt = 0;
  uint8_t nfc_err_max = 2;
  uint8_t nfc_err_state = 2;
  portMUX_TYPE NFCmux = portMUX_INITIALIZER_UNLOCKED;
...
#define sda_io_pin 1
#define scl_io_pin 2
#define pn532_rst_io_pin 42
#define pn532_irq_io_pin 41
...
#define _BV(bit) (1 << (bit))
  ...
  void writecommand(uint8_t *cmd, uint8_t cmdlen)
  {
    ...
    ESP_LOGD("PN532", "%s Sending :", __func__);
    esp_log_buffer_hex("PN532", command, cmdlen + 9);
#endif
    ...
    if (result != ESP_OK)
    {
      ESP_LOGI("error", "i2c %d", PN532ReadState);
      PN532ReadState = FAULT;
      ESP_LOGI("error", "i2c %d", PN532ReadState);
      nfc_err_state = 4;
      ...
      i2c_reset_tx_fifo(PN532_I2C_PORT);
      i2c_reset_rx_fifo(PN532_I2C_PORT);
      resetPN532();
      PN532ReadState = CHECK_FIRMWARE;
    }
  ...
  {
    ets_printf("interrupt being called...");
  ...
  bool init_PN532_I2C(uint8_t sda, uint8_t scl, uint8_t reset, uint8_t irq, i2c_port_t i2c_port_number)
  {
    ...
    gpio_isr_handler_add((gpio_num_t)IRQ_PIN, IRQHandler, (void *)IRQ_PIN);
#endif
    i2c_config_t conf;
    // Open the I2C Bus
...
#ifdef CONFIG_PN532DEBUG
    ESP_LOGD("PN532", "Reading: ");
    esp_log_buffer_hex("PN532", buffer, n + 3);
#endif
    free(buffer);
  ...
  bool isready()
  {
    ...
    uint8_t x = gpio_get_level((gpio_num_t)IRQ_PIN);
#ifdef CONFIG_IRQDEBUG
    ESP_LOGI("PN532", "IRQ: %d", x);
#endif
    return (x == 0);
  }
  ...
  bool waitready(uint16_t timeout)
  {
    ...
    while (!isready())
    {
      if (timeout != 0)
      {
        ...
        {
#ifdef CONFIG_PN532DEBUG
          ESP_LOGE("PN532", "Waitready TIMEOUT after %d ms!", timeout);
#endif
          return false;
        }
      }
  ...
  bool sendCommandCheckAck(uint8_t *cmd, uint8_t cmdlen, uint16_t timeout)
  {
    ...
    {
#ifdef CONFIG_PN532DEBUG
      ESP_LOGD("PN532", "No ACK frame received! Try again");
#endif
      return false;
    }
   ...
   */
  /**************************************************************************/
  /*uint32_t*/ uint8_t getPN532FirmwareVersion(void)
  {
    uint32_t response;
    ...
    {
#ifdef CONFIG_PN532DEBUG
      ESP_LOGD("PN532", "Firmware does not match!");
#endif
      return 0;
    }
...
#ifdef CONFIG_PN532DEBUG
    ESP_LOGD("PN532", "Writing P3 GPIO: 0x%.2X", pn532_packetbuffer[1]);
#endif
...
#ifdef CONFIG_PN532DEBUG
    ESP_LOGD("PN532", "Received: 0x%.2X 0x%.2X 0x%.2X 0x%.2X 0x%.2X 0x%.2X 0x%.2X 0x%.2X", pn532_packetbuffer[0], pn532_packetbuffer[1], pn532_packetbuffer[2], pn532_packetbuffer[3], pn532_packetbuffer[4], pn532_packetbuffer[5], pn532_packetbuffer[6], pn532_packetbuffer[7]);
#endif
  ...
  uint8_t readGPIO(void)
  {
...
#ifdef CONFIG_PN532DEBUG
    printf("Received: ");
    esp_log_buffer_hex("PN532", pn532_packetbuffer, 11);
    printf("\n");
    ESP_LOGD("PN532", "P3 GPIO: 0x%.2X", pn532_packetbuffer[p3offset]);
    ESP_LOGD("PN532", "P7 GPIO: 0x%.2X", pn532_packetbuffer[p3offset + 1]);
    ESP_LOGD("PN532", "P10 GPIO: 0x%.2X", pn532_packetbuffer[p3offset + 2]);
    ...
    case 0x00: // Using UART
      ...
      ESP_LOGD("PN532", "Using UART (IO = 0x00)");
      break;
    case 0x01: // Using I2C
      ESP_LOGD("PN532", "Using I2C (IO = 0x01)");
      break;
    case 0x02: // Using SPI
      ESP_LOGD("PN532", "Using SPI (IO = 0x02)");
      break;
    }
#endif
  ...
  bool setPassiveActivationRetries(uint8_t maxRetries)
  {
    ...
    ESP_LOGD("PN532", "Setting MxRtyPassiveActivation to %d", maxRetries);
#endif
  ...
  bool readPassiveTargetID(uint8_t cardbaudrate, uint8_t *uid, uint8_t *uidLength, uint16_t timeout)
  {
    ...
    {
#ifdef CONFIG_PN532DEBUG
      ESP_LOGD("PN532", "No card(s) read");
#endif
      ESP_LOGI("PN532", "No card(s) read");
      return 0x0; // no cards read
    }

#ifdef CONFIG_PN532DEBUG
    ESP_LOGD("PN532", "Waiting for IRQ (indicates card presence)");
#endif
    if (!waitready(timeout))
    {
#ifdef CONFIG_PN532DEBUG
      ESP_LOGD("PN532", "IRQ Timeout");
#endif
      return 0x0;
    }
...
#ifdef CONFIG_MIFAREDEBUG
    ESP_LOGD("PN532", "Found %d tags", pn532_packetbuffer[7]);
#endif
    if (pn532_packetbuffer[7] != 1)
    {
      ESP_LOGI("PN532", "pn532_packetbuffer[7] != 1");
      return 0;
    }
...
#ifdef CONFIG_MIFAREDEBUG
    ESP_LOGD("PN532", "ATQA: 0x%.2X", sens_res);
    ESP_LOGD("PN532", "SAK: 0x%.2X", pn532_packetbuffer[11]);
#endif
  ...
  bool inDataExchange(uint8_t *send, uint8_t sendLength, uint8_t *response, uint8_t *responseLength)
  {
    ...
    {
#ifdef CONFIG_PN532DEBUG
      ESP_LOGD("PN532", "APDU length too long for packet buffer");
#endif
      return false;
    }
    uint8_t i;
    ...
    {
#ifdef CONFIG_PN532DEBUG
      ESP_LOGD("PN532", "Could not send APDU");
#endif
      return false;
    }
    ...
    {
#ifdef CONFIG_PN532DEBUG
      ESP_LOGD("PN532", "Response never received for APDU...");
#endif
      return false;
    }
    ...
    if (pn532_packetbuffer[0] == 0 && pn532_packetbuffer[1] == 0 && pn532_packetbuffer[2] == 0xff)
    {
      ...
      {
#ifdef CONFIG_PN532DEBUG
        ESP_LOGD("PN532", "Length check invalid 0x%.2X 0x%.2X", length, (~length) + 1);
      ...
      if (pn532_packetbuffer[5] == PN532_PN532TOHOST && pn532_packetbuffer[6] == PN532_RESPONSE_INDATAEXCHANGE)
      {
        ...
        {
#ifdef CONFIG_PN532DEBUG
          ESP_LOGD("PN532", "Status code indicates an error");
#endif
          return false;
        }
      ...
      {
        ESP_LOGD("PN532", "Don't know how to handle this command: 0x%.2X", pn532_packetbuffer[6]);
        return false;
      }
    }
    else
    {
      ESP_LOGD("PN532", "Preamble missing");
      return false;
    }
  }
  ...
  bool inListPassiveTarget()
  {
...
#ifdef CONFIG_PN532DEBUG
    ESP_LOGD("PN532", "About to inList passive target");
#endif

    if (!sendCommandCheckAck(pn532_packetbuffer, 3, I2C_WRITE_TIMEOUT))
    {
#ifdef CONFIG_PN532DEBUG
      ESP_LOGD("PN532", "Could not send inlist message");
#endif
      return false;
    }
    ...
    if (pn532_packetbuffer[0] == 0 && pn532_packetbuffer[1] == 0 && pn532_packetbuffer[2] == 0xff)
    {
      ...
      if (pn532_packetbuffer[5] == PN532_PN532TOHOST && pn532_packetbuffer[6] == PN532_RESPONSE_INLISTPASSIVETARGET)
      {
        ...
        {
#ifdef CONFIG_PN532DEBUG
          ESP_LOGD("PN532", "Unhandled number of targets inlisted");
#endif
          ESP_LOGI("PN532", "Number of tags inlisted: %d", pn532_packetbuffer[7]);
          return false;
        }

        _inListedTag = pn532_packetbuffer[8];
        ESP_LOGI("PN532", "Tag number: %d", _inListedTag);
      ...
      {
#ifdef CONFIG_PN532DEBUG
        ESP_LOGD("PN532", "Unexpected response to inlist passive host");
#endif
        return false;
      }
    }
  ...
  uint8_t mifareclassic_AuthenticateBlock(uint8_t *uid, uint8_t uidLen, uint32_t blockNumber, uint8_t keyNumber, uint8_t *keyData)
  {
...
#ifdef CONFIG_MIFAREDEBUG
    ESP_LOGD("PN532", "Trying to authenticate card ");
    esp_log_buffer_hex("PN532", _uid, _uidLen);
    ESP_LOGD("PN532", "Using authentication KEY %c :", keyNumber ? 'B' : 'A');
    esp_log_buffer_hex("PN532", _key, 6);
#endif
    ...
    {
#ifdef CONFIG_PN532DEBUG
      ESP_LOGD("PN532", "Authentification failed: ");
      esp_log_buffer_hex("PN532", pn532_packetbuffer, 12);
#endif
      return 0;
    }
  ...
  {
#ifdef CONFIG_MIFAREDEBUG
    ESP_LOGD("PN532", "Trying to read 16 bytes from block %d", blockNumber);
#endif
    ...
    {
      //#ifdef CONFIG_MIFAREDEBUG
      ESP_LOGI("PN532", "Failed to receive ACK for read command");
      //#endif
      return 0;
    }
    ...
    {
#ifdef CONFIG_MIFAREDEBUG
      ESP_LOGD("PN532", "Unexpected response");
      esp_log_buffer_hex("PN532", pn532_packetbuffer, 26);
#endif
      return 0;
    }
...
#ifdef CONFIG_MIFAREDEBUG
    ESP_LOGD("PN532", "Block %d", blockNumber);
    esp_log_buffer_hex("PN532", data, 16);
#endif
  ...
  {
#ifdef CONFIG_MIFAREDEBUG
    ESP_LOGD("PN532", "Trying to write 16 bytes to block %d", blockNumber);
#endif
    ...
    {
#ifdef CONFIG_MIFAREDEBUG
      ESP_LOGD("PN532", "Failed to receive ACK for write command");
#endif
      return 0;
    }
    vTaskDelay(10 / portTICK_PERIOD_MS);
  ...
  uint8_t mifareclassic_WriteNDEFURI(uint8_t sectorNumber, uint8_t uriIdentifier, const char *url)
  {
    ...
    uint8_t sectorbuffer1[16] =
        {0x00, 0x00, 0x03, (uint8_t)(len + 5), 0xD1, 0x01, (uint8_t)(len + 1), 0x55, uriIdentifier, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    uint8_t sectorbuffer2[16] =
  ...
  uint8_t mifareultralight_ReadPage(uint8_t page, uint8_t *buffer)
  {
    ...
    {
#ifdef CONFIG_MIFAREDEBUG
      ESP_LOGD("PN532", "Page value out of range");
#endif
      return 0;
    }

#ifdef CONFIG_MIFAREDEBUG
    ESP_LOGD("PN532", "Reading page %d", page);
#endif
    ...
    {
#ifdef CONFIG_MIFAREDEBUG
      ESP_LOGD("PN532", "Unexpected response reading block: ");
      esp_log_buffer_hex("PN532", pn532_packetbuffer, 26);
#endif
      return 0;
    }
    ...
    /* Display data for debug if requested */
#ifdef CONFIG_MIFAREDEBUG
    ESP_LOGD("PN532", "Page %d", page);
    esp_log_buffer_hex("PN532", buffer, 4);
#endif
...
#ifdef CONFIG_MIFAREDEBUG
    ESP_LOGD("PN532", "Trying to write 4 byte page %d", page);
#endif
  ...
  {
    uint8_t success;
    // ESP_LOGI("PN532","PN532_read()");
    switch (PN532ReadState)
    {

    case CHECK_FIRMWARE:
    {
      // ESP_LOGI("PN532","CHECK_FIRMWARE");
      // uint8_t tries=0;
      for (int i = 0; i < 3; i++)
      {
        success = getPN532FirmwareVersion();
      }

      if (success == PROCESS_OK)
      {
        /*NFCStateNum = 1;
        NFCArray[1] = 2;
        firmReadState = 0;*/
        nfc_err_cnt = 0;
        if (xSemaphoreTake(readStateMutex, 0) == pdTRUE)
        {
          PN532ReadState = CONFIGURE_PN532;
          xSemaphoreGive(readStateMutex);
        }

        //  Serial.println("CHECK_FIRMWARE");
      }
      else if (success == PROCESS_NOT_OK)
      {
        /*NFCStateNum = 2;
        NFCArray[2] = 3;
        firmReadState = 0;*/
        nfc_err_cnt += 1;
        if (nfc_err_cnt > nfc_err_max)
        {
          if (xSemaphoreTake(readStateMutex, 0) == pdTRUE)
          {
            PN532ReadState = FAULT;
    ...
    case CONFIGURE_PN532:
    {
      ESP_LOGI("PN532", "CONFIGURE_PN532");
      success = SAMConfig();
      if (success == PROCESS_OK)
      {
        nfc_err_cnt = 0;
        if (xSemaphoreTake(readStateMutex, 0) == pdTRUE)
        {
          PN532ReadState = lIST_AVAILABLE_CARD;
          xSemaphoreGive(readStateMutex);
        }
        vTaskDelay(10);
      }
    ...
    case lIST_AVAILABLE_CARD:
    {
      // ESP_LOGI("PN532","lIST_AVAILABLE_CARD ");

      success = readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, &uidLength, -1);

      ESP_LOGI("PN532", "lIST_AVAILABLE_CARD %d", success);
      if (success == PROCESS_OK)
      {
        nfc_err_cnt = 0;
        if (xSemaphoreTake(readStateMutex, 0) == pdTRUE)
        {
          PN532ReadState = AUTHANTICATE;
          xSemaphoreGive(readStateMutex);
        }
    ...
    case AUTHANTICATE:
    {
      ESP_LOGI("PN532", "AUTHANTICATE");
      // keyA of the NFC card
    ...
    case DATA_READ:
    {
      ...
      ESP_LOGI("PN532", "DATA_READ");
      success = mifareclassic_ReadDataBlock(uidLength, data);
    ...
    case END_READ:
    {
      ESP_LOGI("PN532", "END_READ");
      // PN532ReadState = lIST_AVAILABLE_CARD;
      // the state will change to lIST_AVAILABLE_CARD in state machine after reading userid
      memset(userRefID, '\0', sizeof(userRefID));
      ESP_LOGI("PN532", "before id no:- %s %s", userRefID, userRefIDEmpty);
      uint8_t i = 0;
      uint8_t j = 0;
      while (i < 8)
      {
        itoa((int)data[i], userRefID + j, 10);
        // ESP_LOGI("PN532","i %d j %d data[i] %d ",i,j,data[i]);
        if (data[i] <= 9)
        {
      ...
      if (xSemaphoreTake(readStateMutex, 0) == pdTRUE)
      {
        PN532ReadState = NFC_READY;
        xSemaphoreGive(readStateMutex);
      }
      /*ESP_LOGI("PN532","Reading Block i:4");
      ESP_LOGI("PN532","data %d %d %d %d",data[0],data[1],data[2],data[3]);
      ESP_LOGI("PN532","data %d %d %d %d",data[4],data[5],data[6],data[7]);
      ESP_LOGI("PN532","data %d %d %d %d",data[8],data[9],data[10],data[11]);
      ESP_LOGI("PN532","data %d %d %d %d",data[12],data[13],data[14],data[15]);*/
      ESP_LOGI("PN532", "id no:- %s", userRefID);
    }
    break;

    case FAULT:
    {
      ESP_LOGI("PN532", "FAULT");
      switch (nfc_err_state)
      {
      case 0:
      {
        ESP_LOGI("PN532", "Firmware Fault");
      }
      break;
      case 1:
      {
        ESP_LOGI("PN532", "Configure Fault");
      }
      break;
      case 2:
      {
        ESP_LOGI("PN532", "Auth Fault");
      }
      break;
      case 3:
      {
        ESP_LOGI("PN532", "Read Block Access Fault");
      }
      break;
      case 4:
      {
        ESP_LOGI("PN532", "I2C read fault");
      }
      break;
      default:
      {
        ESP_LOGI("PN532", "Unhandled NFC error state");
      }
      break;
      }
      // resetPN532 ();
    ...
    break;
    default:
      ESP_LOGI("PN532", "Unhandlled state");
      if (xSemaphoreTake(readStateMutex, 0) == pdTRUE)
      {
        PN532ReadState = lIST_AVAILABLE_CARD;
        xSemaphoreGive(readStateMutex);
